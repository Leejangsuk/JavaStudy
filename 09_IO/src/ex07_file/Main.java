...
public class Main {
	...
	public static void m1() {
		...
		System.out.println("수정한날짜 : " + file.lastModified()); // timestampㅌ
	...
	public static void m2() {
		...
		// 존재하지 않으면 디렉터리 생성하기
		if(!dir.exists()) { // if(dir.exists() == flase){
			dir.mkdir(); // 디렉터리 생성하기
	...
	public static void quiz1() {
		...
		// 수정한 날짜		파일크기	파일명
		for(File file : files) {
			System.out.println(new SimpleDateFormat("yyyy-MM-dd a hh:mm").format(file.lastModified()));
			System.out.println("     ");
			if(file.isDirectory())
	...
	public static void quiz2() {
		...
		if(file.exists()) {
			...
			System.out.println(filename + " 파일이 존재하기 않습니다.");
		}
		sc.close();
	}
	...
	File dir = new File("C:\\storage\\m.txt");
