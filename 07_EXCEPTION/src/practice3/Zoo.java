...
public class Zoo {
	...
	public void removeAnimal() throws RuntimeException{
		...
		for(int i = 0; i < idx; i ++) {
			if(name.equals(animals[i].getName())) {
				...
				animals[--idx] = null; // idx를 먼저 감소후 null 값 넣어주기
				return;
			}
		}
	...
	public void manage() {
	 ...
	 while(true) {
		 try {
			 menu();
			 System.out.println("선택(1,2,3,4,0) >>> ");
			 switch(sc.nextInt()){
				 ...
				 default: System.out.println("잘못된 선택 입니다.");

			 }
		 }catch(InputMismatchException e) {
			 System.out.println("선택은 1,2,3,4,0 중 하나 입니다.");
