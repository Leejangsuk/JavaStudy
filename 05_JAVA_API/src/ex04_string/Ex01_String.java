...
public class Ex01_String {
	...
	public static void main(String[] args) {
		...
		// 5. 특정 문자열의 인덱스 반환
		//	  1) indexOf : 발견된 첫 번재 인덱스 반환
		//	  2) lastInedxOf : 발견된 마지막 인덱스
		String str6 = "java";
		int idx1 = str6.indexOf("j");
		int idx2 = str6.indexOf("a");
		int idx3 = str6.indexOf("z");
		
		
		System.out.println(idx1); // 0
		System.out.println(idx2); // 1 (발견된 첫 번재 인덱스 반환
		System.out.println(idx3); // -1 (없는 경우)
		...
		for(int i = 0; i < notices.length; i++) {
			if(notices[i].startsWith("[필독]")){ // [필독]으로 시작하면 true, 아니면 flase
